...
for st = 1:1:length(snr_pt)
    ...
    for ALL = 1:1:50
        ...
        sta_x = randi([-20 40],num(7),1);
        sta_y = randi([-10 50],num(7),1);
        sta = [sta_x, sta_y];
        ...
        %% Add Noise
        rn = awgn(r,snr_pt(st),'measured');
        rmn = awgn(rm,snr_pt(st),'measured');
...
figure;
plot(snr_pt,error_snr,'-o', 'Color',[0.5,0,0.5],'LineWidth',2);
ylabel('mean error / m')
xlabel('SNR / dB')
hold on;
grid on
% legend('5 points','7 points','10 points','13 points')
